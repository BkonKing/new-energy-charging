			...
			<view class="r-dtpicker" :class="{ 'r-dtpicker-show': showPicker }">
				...
				<view class="rangeBox" @touchmove.stop.prevent="returnHandle" @tap.stop="returnHandle">
					<input
						...
						placeholder="开始时间"
						:value="startDate"
						:style="{ color: themeColor, 'border-color': themeColor, opacity: dateType == 'startDate' ? 1 : 0.5 }"
					...
					<input
						...
						disabled
						placeholder="结束时间"
						:style="{ color: themeColor, 'border-color': themeColor, opacity: dateType == 'endDate' ? 1 : 0.5 }"
						:value="endDate"
						@tap="changeDateType('endDate')"
					/>
				</view>
				<picker-view indicator-style="height: 60px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChangeMul">
					<block>
						<picker-view-column>
...
export default {
	...
	methods: {
		...
		init() {
			...
			if (this.value[0]) {
				if (
					(this.fields == 'year' && this.value[0].length != 4) ||
					(this.fields == 'month' && this.value[0].length != 7) ||
				...
				if (this.value[1]) {
					if (
						(this.fields == 'year' && this.value[1].length != 4) ||
						(this.fields == 'month' && this.value[1].length != 7) ||
		...
		changeDateType(dateType) {
			...
			if (this[dateType]) {
				...
				var dateTxt =
					this.fields == 'year'
						? this.yearArr[this.pickerValue[0]]
						: this.fields == 'month'
						? this.yearArr[this.pickerValue[0]] + '-' + this.monthArr[this.pickerValue[1]]
						: this.yearArr[this.pickerValue[0]] + '-' + this.monthArr[this.pickerValue[1]] + '-' + this.dayArr[this.pickerValue[2]];
				this[dateType] = dateTxt;
				this.pickerValue =
					this.fields == 'year'
						? [this.pickerValue[0]]
						: this.fields == 'month'
						? [this.pickerValue[0], this.pickerValue[1]]
