	...
	<view style="width: 100%;">
		<view class="screen-bar" :style="{ height: height + 'rpx', backgroundColor: bgcolor }">
			...
			>
				...
				<image :src="arrBase64" :style="{ transform: show ? (currentIndex == index ? 'rotate(180deg)' : 'rotate(0)') : 'rotate(0)' }"></image>
			</view>
			<view
				class="dropdown-box"
				:style="{
			...
			>
				<scroll-view scroll-y="true" style="height: 100%;">
					<view
						...
						v-for="(item, index) in itemArr[currentIndex]"
						:key="index"
						@tap.stop="subItemClick(index)"
					>
						{{ item[showTag] }}
					</view>
				</scroll-view>
