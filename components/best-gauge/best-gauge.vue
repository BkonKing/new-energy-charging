<template>
  <view class="gauge-box" :style="{ 'background-color': bgColor }">
    <!-- https://ext.dcloud.net.cn/plugin?id=629 -->
    <canvas
      :id="config.id"
      :canvas-id="config.id"
      type="2d"
      :style="{ width: _width + 'px', height: _width + 'px' }"
    ></canvas>
...
export default {
  ...
  computed: {
    ...
    _dStatus: function() {
      return String(this.config.status) === 'true' || this.config.status === 1
        ? 1
        : 0;
    }
  },
  ...
  methods: {
    initCharts() {
      ...
      wx.createSelectorQuery()
        .in(this)
        .select(`#${this.config.id}`) // 在 WXML 中填入的 id
        .node(({ node: canvas }) => {
          uChartsGauge = canvas.getContext('2d');
          const dpr = wx.getWindowInfo().pixelRatio;
          canvas.width = this._width * dpr;
          canvas.height = this._width * dpr; 
          uChartsGauge.scale(dpr, dpr);
    ...
    drawGauge(options, ctx) {
      ...
      for (let idx = 0, len = options.axisTick.length; idx < len; idx++) {
        ...
        var criticalPoint = Math.floor(
          options.value / (options.max / gaugeOption.number)
        ); //大刻度临界点
        var subCriticalPoint = Math.floor(
          options.value /
            (options.max / gaugeOption.subNumber / gaugeOption.number)
        ); //小刻度临界点
        ...
        var childAngle =
          totalAngle / gaugeOption.number / gaugeOption.subNumber;
        var startX = -radius - gaugeOption.width * 0.5;
        var endX = -radius - gaugeOption.width * 0.5 + gaugeOption.width;
        var childEndX =
          -radius - gaugeOption.width * 0.5 + gaugeOption.subWidth;
      ...
      // 数值
      var _fillText =
        options.value * 1 === 0 ? 0 : (options.value * 1).toFixed(1);
      var valueObj = {
    ...
    drawScale(ctx, data) {
      ...
      for (let i = 0; i <= number; i++) {
        ctx.beginPath();
        ctx.strokeStyle = i <= criticalPoint ? colors[0] : colors[1];
    ...
    fillInData: function() {
      ...
      for (var _k in detailDefault) {
        gaugeOption.detail[_k] = this.deepClone(
          detailDefault[_k],
          gaugeOption.detail[_k]
        );
      }
      if (
        gaugeOption.axisTickLength === undefined ||
        gaugeOption.axisTickLength === null
      ) {
      ...
      var axisTickLength =
        gaugeOption.axisTickLength || gaugeOption.axisTick.length || 1;
