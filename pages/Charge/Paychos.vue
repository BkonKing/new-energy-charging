<template>
	<view class="container">
		<view class="PcoA">
			<view class="zduan">
				<view class="zdshop ellipsis" @click="sitedetail">
					...
					<text>福州市规划设计研究院充电桩福州市规划设计研究院充电桩</text>
				</view>
				<view class="zdcode">
					<text>终端编号：1321321131</text>
					<text class="zdcope">复制</text>
				</view>
				<view class="zdtime">当前时段：14:30-17:30</view>
				<view class="zdprice">
					<text>1.0246</text>
					元/度
				</view>
				<view class="zdtime">
					<text>电费：0.8745元/度</text>
					<text>服务费：0.2600元/度</text>
				</view>
		...
		<view class="odtitem">
			...
			<view class="otkind">
				<text v-for="(item, index) in paykind" :key="index" :class="paykindcurrent == index ? 'otkindck' : ''" @click="paykindcheck(index)">{{ item }}</text>
			</view>
			<view class="otli">
				<text>电站优惠</text>
		...
		<view class="odtitem">
			<view class="otli">
				<view>充电策略</view>
				<view class="otcl">
					<text v-for="(item, index) in planCd" :key="index" :class="planCdcurrent == index ? 'otcl_a' : ''" @click="planCdcheck(index)">{{ item }}</text>
				</view>
			</view>
			<view class="otli" v-if="planCdcurrent == 1">
				...
				<view class="otxip"><input placeholder="请输入充电金额(￥)" type="digit" /></view>
			</view>
		</view>
		<!-- 结算策略 -->
		<!-- <view class="odtitem">
		...
		<view class="wfoot"><button class="combutton" @click="charge">启动充电</button></view>
	</view>
</template>
